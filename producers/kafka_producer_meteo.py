#!/usr/bin/env python3
"""Producteur Kafka d'observations meteo simulees."""
...
from spark.spark_utils import load_env_file, project_path, sample_meteo, write_jsonl
...
def main() -> int:
    ...
    parser.add_argument("--topic", default="secur_meteo")
    ...
    for index in range(max_messages):
        record = sample_meteo(index=index, rng=rng)
        rows.append(record)
        if producer:
    ...
    write_jsonl(project_path("data", "raw", "meteo_demo.jsonl"), rows)
    return 0
